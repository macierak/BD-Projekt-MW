...
app.post('/login' , (req, res) => {
    ...
    let cred = { login: req.body.login, pwd: req.body.pwd }
...
app.get("/deaths/soonest", (request, response) => {
    let query = "select imie, nazwisko, date_format(data_śmierci, '%e-%m-%Y') as data_pogrzebu  from zmarły where data_śmierci < curdate()  order by data_śmierci desc"
...
app.get("/raportData/:param", (req, res) => {
    ...
    switch(req.params.param){
        ...
        case "pogrzeby":
            ...
            break
        case "najlepsi":
            query = "select p.nazwisko, count(*) from pogrzeb_pracownik left join pracownik p on p.id=pogrzeb_pracownik.ID_PRAC group by p.nazwisko order by count(*) desc"
            break
        case "rodziny":
